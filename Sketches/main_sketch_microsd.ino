...
Serial.print(year(tAlarm)); Serial.print(",");                                                    // Print current time
Serial.print(month(tAlarm)); Serial.print(",");                                                   // Print current time
Serial.print(day(tAlarm)); Serial.print(",");                                                     // Print current time
Serial.print(hour(tAlarm)); Serial.print(",");                                                    // Print current time
Serial.print(minute(tAlarm)); Serial.print(",");                                                  // Print current time
Serial.print(second(tAlarm)); Serial.print(",");                                                 // Print current time
delay(25);                                                                                        // Buffer for serial printing
...
datafile.print(year(tNow)); datafile.print(",");                                                  // Print current time
datafile.print(month(tNow)); datafile.print(",");                                                 // Print current time
datafile.print(day(tNow)); datafile.print(",");                                                   // Print current time
datafile.print(hour(tNow)); datafile.print(",");                                                  // Print current time
datafile.print(minute(tNow)); datafile.print(",");                                                // Print current time
datafile.print(second(tNow)); datafile.print(",");                                                // Print current time
...
datafile.print(Rain1); datafile.print(",");                                                       // Print rain gauge 1 count
datafile.print(Rain2); datafile.print(",");                                                       // Print rain gauge 2 count
datafile.print(Rain3); datafile.print(",");                                                       // Print rain gauge 3 count
...
if(ADS0.begin()) {                                                                                // Check if ADS0 begins
  ...
  for (uint8_t ADSCHL = 0; ADSCHL < 4; ADSCHL++) {                                                // Define ADS0 for-loop 
    if(ADS0.isReady()) {                                                                          // Get ADS0 ready
      ...
      datafile.print(ads);                                                                        // Print ADS0 reading  
      if(ADSCHL == 3) { 
        datafile.print(",");
        } else {
        datafile.print(",");  
...
if(ADS1.begin()) {                                                                                // Check if ADS1 begins
  ...
  for (uint8_t ADSCHL = 0; ADSCHL < 4; ADSCHL++) {                                                // Define ADS1 for-loop 
    if(ADS1.isReady()) {                                                                          // Get ADS1 ready
      ...
      datafile.print(ads);                                                                        // Print ADS1 reading  
      if(ADSCHL == 3) { 
        datafile.print(",");
        } else {
        datafile.print(",");  
...
if(ADS2.begin()) {                                                                                // Check if ADS2 begins
  ...
  for (uint8_t ADSCHL = 0; ADSCHL < 4; ADSCHL++) {                                                // Define ADS2 for-loop 
    if(ADS2.isReady()) {                                                                          // Get ADS2 ready
      ...
      datafile.print(ads);                                                                        // Print ADS2 reading  
      if(ADSCHL == 3) { 
        datafile.print(",");
        } else {
        datafile.print(",");  
...
void printMUX() {
  ...
  if (error) { 
    ...
    datafile.print(temperature); datafile.print(",");                                             // Print temperature
    datafile.print(humidity); datafile.print(",");                                                // Print humidity
    delay(1);                                                                                     // Insert buffer
  } 

  if(bmx280.begin()) {                                                                            // Check for BMP280
    bmx280.measure();                                                                             // Measure pressure
    datafile.print(bmx280.getPressure()); datafile.print(",");                                    // Print pressure 
    delay(1);                                                                                     // Insert buffer 
  }

  if(spl06.begin()) {                                                                             // Check for SPL06-001
    spl06.measure();  
    datafile.print(spl06.pressure() * 0.01, 2); datafile.print(",");
  }

  if(mlx.begin(0x5A)) {                                                                           // Check for MLX90614
    mlx.setUnit(TEMP_C); mlx.read();                                                              // Read temperature in Celsius
    datafile.print(mlx.object(),2); datafile.print(",");                                          // Print temperature
    delay(1);                                                                                     // Insert buffer
  } 
